...
const MeetingRoom: React.FC<MeetingRoomProps> = ({ user }) => {
  ...
  const handleSlotClick = (roomId: string, roomName: string, time: string, isPast: boolean) => {
    ...
    if (existingBooking) {
      ...
      setBookingForm({
        ...
        department: user.department,
        purpose: '',
        endTime: time // 기본값은 시작 시간과 동일 (1시간)
      });
      setModalState({
        isOpen: true,
        type: 'BOOK',
  ...
  const handleConfirmAction = () => {
    ...
    if (modalState.type === 'BOOK') {
      ...
      // 시간 범위 계산
      const startIdx = times.indexOf(modalState.time);
      const endIdx = times.indexOf(bookingForm.endTime);
      ...
      // 시간 범위 내 예약 충돌 확인
      const timeRange = times.slice(startIdx, endIdx + 1);
      const conflicts = timeRange.filter(t =>
        bookings.some(b => b.roomId === modalState.roomId && b.time === t && b.date === currentDateKey)
      );

      if (conflicts.length > 0) {
        alert(`선택한 시간 범위 중 ${conflicts.join(', ')}에 이미 예약이 있습니다.`);
        return;
      }

      // 시간 범위만큼 예약 생성
      const newBookings: Booking[] = timeRange.map(t => ({
        id: `b-${Date.now()}-${t}`,
      ...
      // 같은 예약 그룹의 모든 시간대 찾아서 삭제
      const targetBooking = bookings.find(b => b.id === modalState.bookingId);
      if (targetBooking) {
        // 같은 날짜, 같은 방, 같은 사용자, 같은 목적의 예약 모두 삭제
        setBookings(bookings.filter(b =>
          !(b.roomId === targetBooking.roomId &&
            b.date === targetBooking.date &&
            b.userId === targetBooking.userId &&
            b.purpose === targetBooking.purpose)
        ));
      }
    }
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                      placeholder="예약자 이름"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                      팀명(부서명) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
                      placeholder="회의 목적 및 내용을 입력하세요"
                      rows={3}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-800/50 text-slate-800 dark:text-white font-medium">
                        {modalState.time}
                      </div>
                      ...
                      <select
                        ...
                        className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                      >
                        {times.slice(times.indexOf(modalState.time)).map(t => (
                        ...
                        <td key={time} className="px-1 py-2 align-middle h-16">
                          {isEditing ? (
                            ...
                            <div
                              ...
                              className={`w-full h-full min-h-[3.5rem] rounded-lg border flex flex-col items-center justify-center transition-all shadow-sm relative z-[1] cursor-pointer ${isMyBooking
